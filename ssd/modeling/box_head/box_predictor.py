...
class BoxPredictor(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        self.cfg = cfg
        self.cls_headers = nn.ModuleList() # list of nnmodule parameters
        ...
        for level, (boxes_per_location, out_channels) in enumerate(zip(cfg.MODEL.PRIORS.BOXES_PER_LOCATION, cfg.MODEL.BACKBONE.OUT_CHANNELS)):
            self.cls_headers.append(self.cls_block(level, out_channels, boxes_per_location)) # predict class for each box
            self.reg_headers.append(self.reg_block(level, out_channels, boxes_per_location)) # predict bounding box for each box
    ...
    def reset_parameters(self):
        # go deeper into box predictor module(cls_headers, reg_headers) and initialize
...
class SSDBoxPredictor(BoxPredictor):
    def cls_block(self, level, out_channels, boxes_per_location): # CLASSIFICATION
        return nn.Conv2d(out_channels, boxes_per_location * self.cfg.MODEL.NUM_CLASSES, kernel_size=3, stride=1, padding=1)

    def reg_block(self, level, out_channels, boxes_per_location): # BOUNDING BOX REGRESSION
