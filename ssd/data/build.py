...
def make_data_loader(cfg, is_train=True, distributed=False, max_iter=None, start_iter=0):
    ...
    for dataset in datasets:
        ...
        elif shuffle: # TRAIN-PHASE  -> shuffle
            sampler = torch.utils.data.RandomSampler(dataset)
        else: # TEST-PHASE -> non shuffle
        ...
        if max_iter is not None:
            batch_sampler = samplers.IterationBasedBatchSampler(batch_sampler, num_iterations=max_iter, start_iter=start_iter) # keep shuffling to max iter
