...
class Compose(object):
    ...
    def __call__(self, img, boxes=None, labels=None):
        """
        convertfromInts(image np.float32), 
        ToPercentCoords(box normalize to image size)
        resize
        subtract_mean
        ...
        for t in self.transforms:
            ...
            if boxes is not None: # DELETE EMPTY BOXES
...
class SubtractMeans(object):
    def __init__(self, mean): # input pixel mean
...
class ToTensor(object):
    def __call__(self, cvimage, boxes=None, labels=None):
        return torch.from_numpy(cvimage.astype(np.float32)).permute(2, 0, 1), boxes, labels # change [h, w, c] -> [c, h, w]
